...
- (void)testInitialContentInset
{
	...
	XCTAssertTrue(UIEdgeInsetsEqualToEdgeInsets([self.scrollView contentInset], expectedInset));
}

- (void)testRotationToLandscape
{
	[self rotateScrollToLandscapeOrientation];
	XCTAssertTrue(CGRectEqualToRect([self.scrollView frame], self.landscapeFrame));
	XCTAssertTrue(UIEdgeInsetsEqualToEdgeInsets([self.scrollView contentInset], self.landscapeAutomaticInset));
}
